...
const NewsItem = (props) => {
  let { title, description, urlToImage, publishedAt, url, author, source } = props;

  let calcTime = {
    publishedTime: new Date(publishedAt).toLocaleString(),
    currentTime: new Date().toLocaleString(),
    timeDiff: Math.abs(new Date(publishedAt).getTime() - new Date().getTime()),
    timeDiffInMinutes: Math.abs(Math.round((new Date(publishedAt).getTime() - new Date().getTime()) / 60000)),
    timeDiffInHours: Math.abs(Math.round((new Date(publishedAt).getTime() - new Date().getTime()) / 3600000)),
    timeDiffInDays: Math.abs(Math.round((new Date(publishedAt).getTime() - new Date().getTime()) / 86400000)),
  };
  ...
  return (
    <Card style={{ margin: "auto" }}>
      ...
      />
      <Card.Body>
        <Badge bg='primary' style={{ margin: "0 0 1rem 0" }}>
